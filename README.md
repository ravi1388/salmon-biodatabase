...
Raw Data Storage: kokanee (Data Lake)

Transformation Logic: biodata-utils (R Package)

Clean Data Storage: sockeye (DBMS)

Analysis Engine: biodata-explorer (R Package)

Presentation Layer: shiny-biodata (Shiny Application)
...
Structure: Implement Hive-style partitioning for optimal query performance, typically by time and species: kokanee/raw_data/species={SPP}/year={YYYY}/data.parquet.
...
Function Design: Functions should be pipe-friendly (data.frame -> data.frame).

DB Interaction: Use the DBI package for consistent database connections.
...
Performance: Manage expensive computations using eventReactive() and implement bindCache() for plots and tables to drastically improve user experience and speed.
